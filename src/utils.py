...
# =============================================================================
def diversity(Z, p, q):
  """Compute the diversity of a community according to the order p
  ...
  Z : numpy matrix
      similarity matrix
  p : numpy array
      relative weight of the individual species
  q : int
      order of diversity
  ...
  diversity : float
  """
  # TODO : the case =1 is not yet sensible!
  if q == 1: # np.exp(diversity_index(Z, p, q))
    ar = np.array((Z @ p) ** p)
    return 1 / np.prod(ar)
  else:
    return ((p * (Z @ p) ** (q - 1)).sum()) ** (1 / (1 - q))


def diversity_index(Z, p, q):
  """Compute the diversity of a community according to the order p
  ...
  if q == 1:
    return - (p * np.log(Z @ p)).sum()
  else:
    return 1 / (q - 1) * (1 - (p * (Z @ p) ** (q - 1)).sum())

...
def plot_ilr_X_vs_Y(X_sim_ilr, Y_sim, Y_star):
    ...
    for i in range(X_sim_ilr.shape[1]):
        if nter < 5:
            fig.add_trace(
                go.Scatter(x=X_sim_ilr[:, i], y=Y_sim, mode="markers", name="Simulated data", marker_color=colours[1],
                           showlegend=(False if i > 0 else True)),
                row=1, col=i + 1
            )

            fig.add_trace(
                go.Scatter(x=X_sim_ilr[:, i], y=Y_star, mode="markers", name="True Effect",
                           opacity=.25, marker_color=colours[2], showlegend=(False if i > 0 else True)),
                row=1, col=i + 1
            )
            nter+=1
...
def plot_mse_results(df_mse,
    ...
    fig.add_trace(
        go.Box(x=df_mse["Method"][mask], y=df_mse["MSE"][mask])
...
def plot_diversity_methods(x, y, xstar, xstar_bound, ystar_ols, ystar_2sls, ystar_kiv, results, ytrue=None):
    ...
    fig.add_trace(
        go.Scatter(x=xstar, y=ystar_ols, name="OLS", mode="lines", line=dict(dash="dot"))#, line_color=colours[0])
    )

    fig.add_trace(
        go.Scatter(x=xstar, y=ystar_2sls, name="2SLS", mode="lines", line=dict(dash="dash"))#, line_color=colours[1])
    )
    fig.add_trace(
        go.Scatter(x=xstar, y=ystar_kiv, name="KIV", mode="lines", line=dict(dash="dash"))#, line_color=colours[2])
    )

    if results is not None:
        fig.add_trace(
            go.Scatter(x=xstar_bound, y=results["objective"][:, 0], name="GB lower",)# line_color=colours[3])
        )

        fig.add_trace(
            go.Scatter(x=xstar_bound, y=results["objective"][:, 1], name="GB upper",)# line_color=colours[4])
